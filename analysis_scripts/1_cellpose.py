...
input_folder = f'python_results/initial_cleanup/8ala_hsp40_hsp70/'
output_folder = f'python_results/cellpose_masking/8ala_hsp40_hsp70/'
...
# clean filenames
img_names = [filename.replace('.tiff', '') for filename in file_list]
...
nuclei_images = [image[:, :, 6] for image in imgs]
...
test = np.load(f'python_results/cellpose_masking/8ala_hsp40_hsp70/cellpose_filtered_masks.npy')
plt.imshow(test[:, :, 0])